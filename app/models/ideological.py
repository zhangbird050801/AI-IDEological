...
class IdeologicalCase(BaseModel, TimestampMixin):
    ...
    course = fields.ForeignKeyField('models.Course', related_name='ideological_cases', null=True, on_delete=fields.SET_NULL)
    chapter = fields.ForeignKeyField('models.Chapter', related_name='ideological_cases', null=True, on_delete=fields.SET_NULL)
    theme_category_id = fields.IntField(null=True, description="思政主题分类ID", index=True)
...
class TeachingResource(BaseModel, TimestampMixin):
    ...
    tags = fields.JSONField(default=list, description="标签列表")
    software_engineering_chapter = fields.CharField(max_length=100, null=True, description="适用章节", index=True)
    course = fields.ForeignKeyField('models.Course', related_name='teaching_resources', null=True, on_delete=fields.SET_NULL)
    chapter = fields.ForeignKeyField('models.Chapter', related_name='teaching_resources', null=True, on_delete=fields.SET_NULL)
    theme_category_id = fields.IntField(null=True, description="思政主题分类ID", index=True)
    usage_count = fields.IntField(default=0, description="使用次数", index=True)
    is_public = fields.BooleanField(default=True, description="是否公开", index=True)
...
class GenerationHistory(BaseModel, TimestampMixin):
    ...
    course = fields.ForeignKeyField('models.Course', related_name='generation_histories', null=True, on_delete=fields.SET_NULL)
    chapter = fields.ForeignKeyField('models.Chapter', related_name='generation_histories', null=True, on_delete=fields.SET_NULL)
